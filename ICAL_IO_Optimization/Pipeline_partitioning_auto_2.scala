...
object Pipeline_partitioning_auto_2 {
  ...
  // at about 8GB.
  var scale_data: Double = 1.0 /40.0;
...
*/
    ...
    val timeslots: RDD[((Int, Int, Int, Int, Int, Int), Data)] = {
      ...
      sc.parallelize(initset,24).map(ix => timeslots_kernel(ix, broads_input0, broads_input1))
    ...
    */
    ...
    val cor_subvis_flag: RDD[((Int, Int, Int, Int, Int, Int),Data)] = {
      ...
      sc.parallelize(initset,20).map(ix => cor_subvis_flag_kernel(ix,broads_input0,broads_input1,broads_input2))
  ...
  //   broads_input2.unpersist()
   //  broads_input1.unpersist()
    // === Gridding Kernel Update + Phase Rotation + Grid + FFT + Reprojection ===
    val grikerupd_pharot_grid_fft_rep: RDD[((Int, Int, Int, Int, Int, Int), Data)] = {
...
//   broads_input.unpersist()
   ...
   //   val input_sum_facets: RDD[((Int, Int, Int, Int), Data)] =
    ...
    //  input_sum_facets.partitionBy(partitioner).mapPartitions(identify_component_kernel, true)
         sum_facets.partitionBy(new SDPPartitioner_facets(81)).mapPartitions(identify_component_kernel)
    }
 ...
 /*def identify_component_kernel : (Iterator[((Int,Int,Int,Int), Data)]) => Iterator[((Int,Int,Int,Int), Data)] = { case (data_sum_facets) =>
  ...
  def identify_component_kernel: (Iterator[((Int, Int, Int, Int,Int,Int), Data)]) => Iterator[((Int, Int, Int, Int), Data)] = {
    case (data_sum_facets) =>
      ...
      result(0) = hash
    //  Thread.sleep((scale_compute * 295259).toInt)
      Iterator(((ix._1,ix._2,ix._3,ix._5), result))
  }
