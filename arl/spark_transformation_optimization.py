...
def SDPPartitioner_by_frequency():
    '''
...
def create_predict_graph_locality(vis_graph_list,gleam_model_graph, broadcast_tele,vis_slices=1, facets=1, context='2d', nfrequency=16, **kwargs):
    results_vis_graph_list = degrid_handle_locality(vis_graph_list,gleam_model_graph, broadcast_tele, context=context, vis_slices=vis_slices,
                                           facets=facets, nfrequency=nfrequency, **kwargs)
...
def create_corrupt_vis_graph_locality(vis_graph_list, gt_graph=None, **kwargs):
    result = corrupt_handle(vis_graph_list, gt_graph, **kwargs)
    return result
...
def create_residual_graph_locality(vis_graph_list, model_graph, context="2d", vis_slices=1, facets=1, nchan=16, **kwargs):
    model_vis = create_zero_vis_graph(vis_graph_list)
    model_vis = create_predict_graph(model_vis, model_graph, vis_slices=vis_slices, facets=facets, context=context, nfrequency=nchan, **kwargs)
    residual_vis = create_subtract_vis_graph_list_locality(vis_graph_list, model_vis, **kwargs)
    return create_invert_graph(residual_vis, model_graph, dopsf=False, normalize=True, context=context, vis_slices=vis_slices, facets=facets, **kwargs)

def create_calibrate_graph_list(vis_graph_list, global_solution=True, **kwargs):
    if global_solution:
        point_vis_graph_list = divide_handle_locality(vis_graph_list, **kwargs)
        ...
        return solve_and_apply_handle(vis_graph_list, **kwargs)
def create_calibrate_graph_list_locality(vis_graph_list, model_vis_graph_list, global_solution=True, **kwargs):
    if global_solution:
        point_vis_graph_list = divide_handle_locality(vis_graph_list, **kwargs)
        point_vis_graph_list = [tup[1] for tup in point_vis_graph_list.collect()]
        global_point_vis_graph = visibility_gather_channel(point_vis_graph_list)
...
def create_deconvolve_graph(sc, dirty_graph, psf_graph, model_graph, **kwargs):
    ...
    if algorithm == "mmclean" and nchan > 1:
        ...
        for i in range(nchan):
            ...
            ret_img.append(create_image_from_array(im.data[channel:channel_max, ...], wcs, im.polarisation_frame))
...
def create_ical_graph_locality(sc,broadcast_tele, vis_graph_list, model_graph, nchan, context="2d", vis_slices=1, facets=1, first_selfcal=None, **kwargs):
...
def create_ical_graph(sc, vis_graph_list, model_graph, nchan, context="2d", vis_slices=1, facets=1, first_selfcal=None, **kwargs):
    ...
    for i in psfs:
        if i[0][2] == 0:
            psf = i[1][0]
    print("   psf     max      here   ",str(qa_image(psf).data['max']))   
    print("   psf     min      here   ",str(qa_image(psf).data['min']))
    print("   psf  graph    ",str(psf_graph.count()))
    if first_selfcal is not None and first_selfcal == 0:
        model_vis_graph_list = create_zero_vis_graph(vis_graph_list)
        model_vis_graph_list = create_predict_graph(model_vis_graph_list, model_graph, vis_slices=vis_slices, facets=facets,
                                                    context=context, nfrequency=nchan, **kwargs)
        vis_graph_list = create_calibrate_graph_list(vis_graph_list, model_vis_graph_list, **kwargs)
        print("  calibrate graph list  ",str(vis_graph_list.count()))
        residual_vis_graph_list = create_subtract_vis_graph_list(vis_graph_list, model_vis_graph_list, **kwargs)
        residual_graph = create_invert_graph(residual_vis_graph_list, model_graph, dopsf=True, context=context, vis_slices=vis_slices, facets=facets, **kwargs)
    ...
    residual, sumwt = (None, None)
    for i in residuals:
        if i[0][2] == 0:
            residual, sumwt = i[1]
    print("   residual     max      here   ",str(qa_image(residual).data['max'])) 
    print("   residual     min      here   ",str(qa_image(residual).data['min']))
    deconvolve_model_graph = create_deconvolve_graph(sc, residual_graph, psf_graph, model_graph, nchan=nchan, **kwargs)
    deconvoles=deconvolve_model_graph.collect()
    dec = None
    for i in deconvoles:
        if i[0][2] == 0:
            dec = i[1]
    print("   deconvoles     max      here   ",str(qa_image(dec).data['max'])) 
    print("   deconvoles     min      here   ",str(qa_image(dec).data['min']))
    ...
    if nmajor > 1:
        for cycle in range(nmajor):
            if first_selfcal is not None and cycle >= first_selfcal:
                #model_vis_graph_list = create_zero_vis_graph(vis_graph_list)
                #put model and vis data together
                #model_vis_graph_list = create_predict_graph(model_vis_graph_list, deconvolve_model_graph, vis_slices=vis_slices, facets=facets,
                model_vis_graph_list = create_predict_graph_locality(vis_graph_list, deconvolve_model_graph,
                                                            vis_slices=vis_slices, facets=facets,
                                                        context=context, nfrequency=nchan, **kwargs)
                print("   modle  graph  ********  ",str(model_vis_graph_list.count()))
                vis_graph_list = create_calibrate_graph_list(vis_graph_list, model_vis_graph_list, **kwargs)
                residual_vis_graph_list = create_subtract_vis_graph_list(vis_graph_list, model_vis_graph_list, **kwargs)
                residual_graph = create_invert_graph(residual_vis_graph_list, model_graph, dopsf=False, context=context, vis_slices=vis_slices, facets=facets, **kwargs)
...
def reppre_ifft_kernel(ix, data_extract_lsm, dep_image, insert_method, applybeam):
    #beam, major_loop, i, time, facet, polarisation
    ...
    print("Here model size   is   "+model.size)
...
def degrid_handle_locality(reppre_ifft, telescope_value, context, vis_slices, facets,first, nfrequency, **kwargs) -> BlockVisibility :
    ...
    if context == "2d":
        ...
        if(first==1):
            ...
            return reppre_ifft.map(
                lambda record: degrid_kernel(record, telescope_value, c["predict"], context=context, **kwargs))
    elif context == "facets":
        telescope_data = telescope_data.mapValues(lambda vis: coalesce_visibility(vis, **kwargs))
        print("scatter_image")
        return reppre_ifft.flatMap(lambda im: scatter_image_flatmap(im, facets=facets, image_iter=c["image_iterator"], **kwargs), True)\
            .map(lambda record: degrid_kernel_2d(record, telescope_value,c["predict"], context=context, **kwargs ), True).reduceByKey(sum_predict_vis_reduce_kernel)

    elif context == "slice" or context == "timeslice" or context == "wstack":
        telescope_data_origin = telescope_data.map(lambda vis: (vis[0], coalesce_visibility(vis[1], **kwargs)))
        telescope_data = telescope_data_origin.flatMap(
            lambda vis: scatter_vis_flatmap(vis, vis_slices=vis_slices, vis_iter=c["vis_iterator"], **kwargs), True)
        return reppre_ifft.map(lambda record: degrid_kernel(record, c["predict"], context=context, **kwargs)) \
        .combineByKey(gather_vis_createCombiner_kernel, gather_vis_mergeValue_kernel, gather_vis_mergeCombiner_kernel)\
        .mapValues(lambda data: gather_vis_kernel(data, vis_slices=vis_slices, vis_iter=c["vis_iterator"], **kwargs))
...
def create_empty_handle(vis_graph_list, npixel, cellsize, frequency, channel_bandwidth, polarisation_frame):
    print("    channel_bandwith    is    ",str(len(channel_bandwidth)))
...
def divide_handle_locality(viss, **kwargs):
    parallelism = get_parameter(kwargs, "parallelism")
    return viss.mapValues(divide_kernel)
...
def solve_and_apply_handle_locaity(viss, model_viss, **kwargs):
    parallelism = get_parameter(kwargs, "parallelism")
    return viss.join(model_viss, parallelism).mapValues(lambda ixs: solve_and_apply_kernel_locality(ixs, **kwargs))
...
def solve_and_apply_kernel_locality(ixs, **kwargs):
    vis, model_vis = ixs
    gt = solve_gaintable(vis, model_vis, **kwargs)
    return (apply_gaintable(vis, gt, inverse=True, **kwargs),model_vis)
