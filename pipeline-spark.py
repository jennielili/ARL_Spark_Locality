...
def trial_case(results, seed=180555, context='wstack', nworkers=8, threads_per_worker=1,
    ...
    gleams=gleam_model_graph.collect()
    #gleam=none
    for i in gleams:
        if i[0][2] == 0:
            gleam = i[1]
    show_image(gleam.data[0, 0, ...], 'hello_gleam', 1)
    #export_images_to_fits(gleams, nfreqwin, "gleam.fits")
    ...
    model_graph.cache()
    models=model_graph.collect()
    #export_images_to_fits(models, nfreqwin, "models.fits")
    #print("****** models  fits  generated  ******")
    ...
    print(results['psf_max'])
    print(results['psf_min'])
    dirty_graph = create_invert_graph(vis_graph_list, model_graph, vis_slices=vis_slices, context=context,
                                      facets=facets,
                                      kernel=kernel)
    start = time.time()
    print("****** Starting dirty image calculation ******")
